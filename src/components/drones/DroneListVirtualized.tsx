...
interface DroneListVirtualizedProps {
  drones: Drone[];
  onSelectDrone: (drone: Drone) => void;
}

const DroneListVirtualized: React.FC<DroneListVirtualizedProps> = ({ 
  drones, 
  onSelectDrone 
}) => {
  ...
  const LoadingComponent = () => (
    <div className="py-8 text-center">
      <div className="inline-flex items-center space-x-2">
        ...
        <span className="text-gray-500">Daha fazla drone yükleniyor...</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="px-6 py-3 border-t border-gray-200 text-sm text-gray-500">
        Toplam {filteredDrones.length} drone gösteriliyor
      </div>
    </div>
  );
};

export default DroneListVirtualized;